...
public class WorkflowInstanceStatusController {
	...
	public String workflowInstanceStatus(@PathVariable("workflowInstanceId") String workflowInstanceId, @PathVariable("userId")String userId, Model model) {
	...
	public ResponseEntity<WorklfowInstanceStatusWrapper> getWorkflowInstanceDetails(
				...
				for (TaskInstance taskInstance : taskInstanceList) {
					...
					if (taskInstance.getStatus().equals("Running")) {
						if (taskInstance.getUser() != null) {
							...
							workflowInstanceStatusWrapper.isAllowed = notificationService.checkUserPermission(taskInstance, userId);
						}
					}
				}
	  ...
	  public ResponseEntity<Void> approveTaskInstance(@PathVariable("taskInstanceId")String taskInstanceId, @PathVariable("userId")String userId){
	  ...
	  public ResponseEntity<Void> rejectTaskInstance(@PathVariable("taskInstanceId")String taskInstanceId, @PathVariable("userId")String userId){
		  int returnCode = taskInstanceService.rejectTaskInstance(Long.parseLong(taskInstanceId), Long.parseLong(userId));
		  if(returnCode == 1) {
			  return new ResponseEntity<Void>(HttpStatus.OK);
		  }
		  return new ResponseEntity<Void>(HttpStatus.INTERNAL_SERVER_ERROR);
